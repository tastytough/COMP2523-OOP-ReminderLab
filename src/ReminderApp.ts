...
export default class ReminderApp {
  ...
  private handleToggleCompletion(): void {
    if(this._remindersHandler.size() == 0) {
      ...
      Logger.logReminders(this._remindersHandler.getReminders());
      const numberToToggle = parseInt(this.getUserChoice("reminder number to toggle", true));
      this._remindersHandler.toggleCompletion(numberToToggle);
  ...
  private handleModifyReminders(): void {
    if(this._remindersHandler.size() == 0) {
      ...
      Logger.logReminders(this._remindersHandler.getReminders());
      const modifyNumberReminder = parseInt(this.getUserChoice("reminder number to edit", true));
      const modifyDescription = this.getUserChoice("description", false);
      this._remindersHandler.modifyReminder(modifyNumberReminder, modifyDescription);
      if (ReminderApp.checkUserToggleChoice() === true) {
        this._remindersHandler.toggleCompletion(modifyNumberReminder);
      }
      Logger.log('\n  🏁   Reminder Modified');
    }
    /*
  ...
  private handleAddReminder(): void {
    const userDescription = this.getUserChoice("reminder", false);
    if (!(userDescription.length === 0)) {
      const userTag = this.getUserChoice("tag", false)
      this._remindersHandler.addReminder(userDescription, userTag);
      Logger.log('\n  🏁  Reminder Added');
    };
  ...
  private handleSearchReminders(): void {
    if(this._remindersHandler.size() == 0) {
      ...
      const searchWord = this._remindersHandler.search(keyword);
      Logger.logSearchResults(searchWord);
    }
    /*
        Pseudocode:
        If no reminders then -> Logger.log('\n  ⚠️  You have no reminders');
  ...
  private handleShowReminders(): void {
    if(this._remindersHandler.size() == 0) {
      ...
      const groupReminder: RemindersGroupingByTag = this._remindersHandler.groupByTag()
      Logger.logGroupedReminders(groupReminder)
    }
    /*
        Pseudocode:
        If no reminders then -> Logger.log('\n  ⚠️  You have no reminders');
